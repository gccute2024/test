 ...
 * @LastEditTime: 2023-07-18 15:33:09
 * @Description: 
 * 
 * 
...
static BYTEP heap_info=&_heap_start+2*sizeof(size_t);
...
void *_sbrk(size_t nbyte)//申请空间
{
    ...
    if(mem_start==NULL)//初始化
    {
            ...
            *heap_info_size=*heap_size/32;//获得信息存储空间大小
            ...
            for(DWORD i=0;i<*heap_info_size;i++)*(heap_info+i)=0;//赋予内容
    ...
    {
        if((*(heap_info+bit/8))&(1<<(bit%8)))cnt=0;
        else cnt++;
        if(cnt*4>=nbyte)
        {
            bit-=(cnt-1);
...
void *malloc_r (size_t nbyte)//数据处理
{
    ...
    if(byte==0||byte<nbyte)return NULL;
    if(byte<8)nbyte=8;//至少申请8个字节
    return _sbrk(byte);
    
}
void *malloc (size_t nbytes)		/* get a block */
{
    if(nbytes==0)return NULL;//防0
...
{
	c&=0xff;
    for(size_t i=0;i<n;i++)*((BYTEP)str+i)=c;
	return str;
}
 void *	 memcpy(void* str1,const void* str2, size_t num)
{
    void* ret = str1;
    BYTEP dest=(char *)str1;
    BYTEP src=(char *)str2;
    if(dest==NULL||src==NULL)return ((void *)-1);
    while (num--)
    {
        *dest = *src;
        dest++;
...
_sbrk (int  incr)
{
	...
	static char *heap_end=NULL;		/* Previous end of heap or 0 if none */
	char        *prev_heap_end;
