 ...
 * @LastEditTime: 2023-07-25 18:15:11
 * @Description: 
 * 
 * 
...
/***************weak声明*********************/
void __interrupt CPUS_HANDLER(void)__weak;
void __interrupt CPUT_HANDLER(void)__weak;  
void __interrupt CPUR_HANDLER(void)__weak;  
void __interrupt SWUC_HANDLER(void)__weak;  
void __interrupt LPCRST_HANDLER(void)__weak; 
void __interrupt PWRSW_HANDLER(void)__weak;  
void __interrupt PS2_0_HANDLER(void)__weak;  
void __interrupt KBS_SDV_HANDLER(void)__weak;
void __interrupt KBS_PRESS_HANDLER(void)__weak; 
void __interrupt TACH0_HANDLER(void)__weak; 
void __interrupt TACH1_HANDLER(void)__weak;  
void __interrupt TACH2_HANDLER(void)__weak;  
void __interrupt TACH3_HANDLER(void)__weak; 
void __interrupt KBC_IBF_HANDLER(void)__weak; 
void __interrupt KBC_OBE_HANDLER(void)__weak; 
void __interrupt PMC1_IBF_HANDLER(void)__weak; 
void __interrupt PMC1_OBE_HANDLER(void)__weak;
#if (defined(TEST101)||defined(AE101)||defined(AE102))
void __interrupt PMC2_IBF_HANDLER(void)__weak; 
void __interrupt PMC2_OBE_HANDLER(void)__weak;
#elif defined(AE103)
void __interrupt WU42_HANDLER(void)__weak; 
void __interrupt  RTC_HANDLER(void) __weak; 
#endif 
void __interrupt WDT_HANDLER(void)__weak;   
void __interrupt ADC_HANDLER(void)__weak;   
void __interrupt UART0_HANDLER(void)__weak; 
void __interrupt UARTA_HANDLER(void)__weak; 
void __interrupt UARTB_HANDLER(void)__weak; 
void __interrupt SM_EC_HANDLER(void)__weak;   
void __interrupt SM_HOST_HANDLER(void)__weak;  
void __interrupt TIMER0_HANDLER(void)__weak; 
void __interrupt TIMER1_HANDLER(void)__weak; 
void __interrupt TIMER2_HANDLER(void)__weak; 
void __interrupt TIMER3_HANDLER(void)__weak; 
void __interrupt INTR0_HANDLER(void)__weak;  
void __interrupt INTR1_HANDLER(void)__weak;  
/***************weak声明*********************/


#if IRQC_DEBUG
char __weak *irq_string="irqc %d\n";
#endif
...
irqprint(irq_string,15);
	...
	//irqprint ("%s","--------PMC1_InputBufferFull\n");
	F_Service_PCI2 = 1;
	//Service_PCI2_Main();
...
DWORD  kernel_trap(uintptr_t mcause, uintptr_t sp)
{
 
  VBYTE trap_flag=0x1f&mcause;
  F_Service_PUTC=1;
  dprint(" \ntrap\nmcause is %d:%s\n mepc is 0x%x,val is 0x%x\n",trap_flag,Exception_Code[trap_flag], read_csr(mepc),(*(BYTEP)read_csr(mepc)));
  F_Service_PUTC=0;
  _exit(mcause);
   return 0;
}
...
void disable_irq(int index,int num)
{
    set_irq_reg(index,IRQ_REG_INTMASK_L, num);
}

void enable_irq(int index,int num)
{
    clear_irq_reg(index,IRQ_REG_INTMASK_L,num);
}

void soc_irq_init()
{
    irq_write(0,IRQ_REG_INTEN_L + 0, 0xff);//enable irq interrupt
    irq_write(0,IRQ_REG_INTEN_L + 1, 0xff);//enable irq interrupt
    irq_write(0,IRQ_REG_INTEN_L + 2, 0xff);//enable irq interrupt
    irq_write(0,IRQ_REG_INTEN_L + 3, 0xff);//enable irq interrupt
    irq_write(0,IRQ_REG_INTEN_L + 4, 0xff);//enable irq interrupt
    irq_write(0,IRQ_REG_INTEN_L + 5, 0xff);//enable irq interrupt
    irq_write(0,IRQ_REG_INTEN_L + 6, 0xff);//enable irq interrupt
    irq_write(0,IRQ_REG_INTEN_L + 7, 0xff);//enable irq interrupt
    irq_write(0,IRQ_REG_INTMASK_L + 0, 0x0); //mask all irq interrupt
    irq_write(0,IRQ_REG_INTMASK_L + 1, 0x0); //mask all irq interrupt
    irq_write(0,IRQ_REG_INTMASK_L + 2, 0x0); //mask all irq interrupt
    irq_write(0,IRQ_REG_INTMASK_L + 3, 0x0); //mask all irq interrupt
    irq_write(0,IRQ_REG_INTMASK_L + 4, 0x0); //mask all irq interrupt
    irq_write(0,IRQ_REG_INTMASK_L + 5, 0x0); //mask all irq interrupt
    irq_write(0,IRQ_REG_INTMASK_L + 6, 0x0); //mask all irq interrupt
    irq_write(0,IRQ_REG_INTMASK_L + 7, 0x0); //mask all irq interrupt

    irq_write(1,IRQ_REG_INTEN_L + 0, 0xff);//enable irq interrupt
    irq_write(1,IRQ_REG_INTEN_L + 1, 0xff);//enable irq interrupt
    irq_write(1,IRQ_REG_INTEN_L + 2, 0xff);//enable irq interrupt

    irq_write(1,IRQ_REG_INTEN_L + 4, 0xff);//enable irq interrupt
    irq_write(1,IRQ_REG_INTEN_L + 5, 0xff);//enable irq interrupt
    irq_write(1,IRQ_REG_INTEN_L + 6, 0xff);//enable irq interrupt
    irq_write(1,IRQ_REG_INTEN_L + 7, 0xff);//enable irq interrupt

    irq_write(1,IRQ_REG_INTMASK_L + 0, 0x0); //mask all irq interrupt
    irq_write(1,IRQ_REG_INTMASK_L + 1, 0x0); //mask all irq interrupt
    irq_write(1,IRQ_REG_INTMASK_L + 2, 0x0); //mask all irq interrupt
    irq_write(1,IRQ_REG_INTMASK_L + 4, 0x0); //mask all irq interrupt
    irq_write(1,IRQ_REG_INTMASK_L + 5, 0x0); //mask all irq interrupt
    irq_write(1,IRQ_REG_INTMASK_L + 6, 0x0); //mask all irq interrupt
    irq_write(1,IRQ_REG_INTMASK_L + 7, 0x0); //mask all irq interrupt

    irq_write(1,IRQ_REG_INTEN_L + 3, 0xff);//enable irq interrupt
    irq_write(1,IRQ_REG_INTMASK_L + 3, 0x0); //mask all irq interrupt

}

void config_irqc_irqs (void)
{
	...
	irqc_enable_interrupt (IRQC_INT_DEVICE_SWUC);
	irqc_enable_interrupt (IRQC_INT_DEVICE_PS2);

    #if 1
	irqc_enable_interrupt (IRQC_INT_DEVICE_KBS_SDV);
	irqc_enable_interrupt (IRQC_INT_DEVICE_KBS_PRESS);
    #endif

	irqc_enable_interrupt (IRQC_INT_DEVICE_TACH0);
	irqc_enable_interrupt (IRQC_INT_DEVICE_TIMER0);
	irqc_enable_interrupt (IRQC_INT_DEVICE_TIMER1);
	irqc_enable_interrupt (IRQC_INT_DEVICE_TIMER2);
	irqc_enable_interrupt (IRQC_INT_DEVICE_TIMER3);

    irqc_enable_interrupt (IRQC_INT_DEVICE_KBC_IBF);
    irqc_enable_interrupt (IRQC_INT_DEVICE_PMC1_IBF);
    irqc_enable_interrupt (IRQC_INT_DEVICE_PMC2_IBF);

    #if 0
    irqc_enable_interrupt (IRQC_INT_DEVICE_PMC2_OBE);
    irqc_enable_interrupt (IRQC_INT_DEVICE_KBC_OBE);
    irqc_enable_interrupt (IRQC_INT_DEVICE_PMC1_OBE);
    #endif

    irqc_enable_interrupt (IRQC_INT_DEVICE_INTR0);
    irqc_enable_interrupt (IRQC_INT_DEVICE_INTR1);
	#if !TEST
	irqc_disable_interrupt(IRQC_INT_DEVICE_KBC_IBF); //disable kbc ibf int
	irqc_disable_interrupt(IRQC_INT_DEVICE_PMC1_IBF); //disable pmc ibf int
	#endif
	#ifdef AE103
	
	irqc_disable_interrupt(IRQC_INT_DEVICE_WU42); //disable wu42 int
	#endif
}
...
{
    //write_csr(0xBD0,0x0);
    write_csr(0xBD1,0xffffffe0);
    //write_csr(0xBD1,0x0);
    //write_csr(0xBD2,0x0);
    write_csr(0xBD2,0xfffffff0);
    //write_csr(0xBD3,0x0);
    //write_csr(0xBD3,0xffffffff);
}
