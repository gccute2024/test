...
peripheral_list subroutine_list[]={//功能链表
    ...
    {.name="I2C1",  .select=0,.succeed=0,.error=0,.id=0,.func=I2C_subroutine,   .IO=PMC5_DR,.SR_CMD=PMC5_SR_CMD,.lock=&PMC5_lock},
    ...
    {.name="CAN0",  .select=0,.succeed=0,.error=0,.id=0,.func=CAN_subroutine,   .IO=PMC1_DR,.SR_CMD=PMC1_SR_CMD,.lock=&PMC1_lock},
    {.name="CAN1",  .select=0,.succeed=0,.error=0,.id=0,.func=CAN_subroutine,   .IO=PMC2_DR,.SR_CMD=PMC2_SR_CMD,.lock=&PMC2_lock},
    {.name="CAN2",  .select=0,.succeed=0,.error=0,.id=0,.func=CAN_subroutine,   .IO=PMC3_DR,.SR_CMD=PMC3_SR_CMD,.lock=&PMC3_lock},
    {.name="CAN3",  .select=0,.succeed=0,.error=0,.id=0,.func=CAN_subroutine,   .IO=PMC4_DR,.SR_CMD=PMC4_SR_CMD,.lock=&PMC4_lock},
};
...
void *I2C_subroutine(void *name)//I2C子程序
{
    ...
    while (1)
    {
        ...
        if(!strcmp(name,"I2C0")) var = data_send_recv(list_cnt, TO_IO, 0x00);
        else if(!strcmp(name,"I2C1")) var = data_send_recv(list_cnt, TO_IO, 0x01);
        pthread_mutex_unlock(subroutine_list[list_cnt].lock);
        printf("temp = %d\n",var);
        // msleep(Ms_sleep,Us_sleep);
        // while (!((*(volatile u8 *)(map18+(subroutine_list[list_cnt].SR_CMD))) & OBFS))
