...
int main(int argc,char **argv)
{
    ...
    while (args--)
    {
        ...
        //打开文件
        if(argc != 1) bin_fd = open(argv[1],O_RDONLY);
        else bin_fd = open("./ec_main.bin",O_RDONLY);
        if(bin_fd == -1)
        {
            if(argc != 1)printf("%s ",argv[1]);
            else printf("./ec_main.bin ");
            perror("open fail");
            close(dev_fd);
            exit(bin_fd);
        ...
        struct stat binstat;
        if(argc == 1)stat("./ec_main.bin",&binstat);
        else stat(argv[1],&binstat);
        ...
        while (cnt <=cnt_max)
        {
            ...
            if(cnt== cnt_max)read(bin_fd,buff,cnt_max_size);
            else read(bin_fd,buff,1024);
            if(cnt== cnt_max)state = 1;
...
rewrite:           
            ...
            do
            {
                ...
                putc(host_uart_rx,stdout);
            putc('\n',stdout);
                if(host_uart_rx == 'E' || host_uart_rx == 'F')
                {
                    while(host_uart_rx != 'A'){write(dev_fd,&error_Update,1);read(dev_fd,&host_uart_rx,1);}
                    goto rewrite;
                }
